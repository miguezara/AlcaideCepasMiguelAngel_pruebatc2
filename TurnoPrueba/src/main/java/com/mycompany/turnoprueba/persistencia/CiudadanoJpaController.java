...
import com.mycompany.turnoprueba.persistencia.exceptions.NonexistentEntityException;
...
public class CiudadanoJpaController implements Serializable {
    ...
    public CiudadanoJpaController(){
        emf = Persistence.createEntityManagerFactory("TurnoPruebaPU");
    }
    ...
    public Optional<Ciudadano> findCiudadanoByDni(String dni) {
    EntityManager em = getEntityManager();
    try {
        Query query = em.createQuery("SELECT c FROM Ciudadano c WHERE c.dni = :dni");
        query.setParameter("dni", dni);
        List<Ciudadano> result = query.getResultList();
